...
### Added
- GitHub CodeQL analysis workflow for the Java codebase.
- Dependabot version updates for Maven dependencies and GitHub Actions.
- Repository `CODEOWNERS` file for protected branch review flows.
- Pull-request CI workflow for validation before merge into `main`.
- Go-based installer with both TUI and CLI interfaces under `installer/` for k3s/Kubernetes installation.
- Interactive namespace detection and Elasticsearch secret provisioning in the installer.
- Expanded Kubernetes manifests for K-Guard AI deployment, service, ConfigMap, and example Secret aligned with v0.8.0.

### Changed
- Updated repository governance for protected `main` branch reviews and required status checks.
- Updated project documentation (README) to reflect the v0.8.0 security hardening milestone and the official installation workflow (installer + manifests).
- Stabilized Spring Boot context loading by providing explicit beans for HTTP clients and JSON mapping (e.g., `RestTemplate`, `ObjectMapper`) used by Elasticsearch export and local LLM enrichment services.

### Security
- Hardened Kubernetes deployment with non-root execution, explicit runtime identity, dropped Linux capabilities, read-only root filesystem, `allowPrivilegeEscalation: false`, and writable `/tmp` through `emptyDir`.
- Added CodeQL-based static analysis to the repository security baseline.
- Added Dependabot update coverage for Maven and GitHub Actions dependency management.
- Kept Elasticsearch export optional and driven by configuration flags, with credentials provided via Kubernetes Secret instead of hard-coded settings.

### Validation
- Verified pull-request CI workflow execution with Maven test validation.
- Verified repository branch protection can require CodeQL results on pull requests.
- Verified repository review ownership structure through `CODEOWNERS`.
- Verified Spring Boot application startup on Java 21 with `KGuardAiApplicationTests.contextLoads`.
- Verified Go installer build and execution (`check`, `install`, `status`) against a Kubernetes cluster using `kubectl`.
